...
Additionally, ensure test coverage doesn't drop below the baseline (85%). Check coverage with:

```bash
cargo tarpaulin --out Stdout --skip-clean
```

New features should include tests that maintain or improve coverage. The 85% baseline focuses on core game logic and real gameplay scenarios rather than error paths for impossible states.
